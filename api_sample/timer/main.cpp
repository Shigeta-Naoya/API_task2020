...
UINT WINAPI TFunc(LPVOID thParam)
{
	...
	***************************************************************/
	...
	while (Flag == TRUE) {
		...
		//表示
		_gcvt_s(str,256,data,8);								//double型を文字列に変換
		...
		//秒数表示
		time++;
		_itoa_s((int)(time / (int)DEF_DATAPERS), str, 10);	//秒数(int型)を文字列に変換
		mbstowcs_s(&ret, wcs1, 256, str, _TRUNCATE);        //charからwcharに変換する
		...
		if (time == 60000) {
			...
			_itoa_s((int)(min), str2, 10);	                 //分数(int型)を文字列に変換
